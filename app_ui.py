...
# Indian languages supported for translation (display name -> ISO 639-1 code)
INDIAN_LANGUAGES = {
    "Hindi": "hi",
    "Marathi": "mr",
    "Bengali": "bn",
...
class SignLanguageApp:
    def __init__(self, window, window_title):
        ...
        if not self.cap.isOpened():
            raise RuntimeError(
                ...
                "not in use by another app, and that camera permissions are granted."
            )

        # Camera failure tracking (used if the camera disconnects mid-session)
        ...
        # Camera status banner (hidden unless something goes wrong)
        self.camera_status_var = ctk.StringVar(value="")
        self.camera_status_label = ctk.CTkLabel(
            self.left_panel, textvariable=self.camera_status_var,
            font=ctk.CTkFont(size=13, weight="bold"), text_color="#ef4444"
        )
        self.camera_status_label.grid(row=2, column=0, sticky="w", pady=(8, 0))

        # --- RIGHT PANEL (Controls & Sidebar) ---
        self.right_frame = ctk.CTkScrollableFrame(self.main_bg, width=380, corner_radius=20,
                                        fg_color=("#ffffff", "#1e1e21"),
                                        border_width=2, border_color=("#d1d5db", "#333333"))
        ...
        # Sentence Textbox
        self.sentence_title = ctk.CTkLabel(self.right_frame, text="Word / Sentence (English)", font=ctk.CTkFont(size=14), text_color="gray")
        self.sentence_title.pack(pady=(10, 5), anchor="w", padx=25)

        self.sentence_text = ctk.CTkTextbox(self.right_frame, height=120, font=ctk.CTkFont(size=22),
        ...
        self.auto_checkbox.pack(pady=(0, 20), padx=25, anchor="w")
        ...
        # ---------------- TRANSLATION SECTION ----------------
        self.translate_divider = ctk.CTkFrame(self.right_frame, height=2, fg_color=("#e5e7eb", "#333333"))
        self.translate_divider.pack(fill="x", padx=25, pady=(25, 15))

        self.translate_section_title = ctk.CTkLabel(self.right_frame, text="Translate", font=ctk.CTkFont(size=20, weight="bold"))
        self.translate_section_title.pack(pady=(0, 15))

        self.lang_title = ctk.CTkLabel(self.right_frame, text="Target Language", font=ctk.CTkFont(size=14), text_color="gray")
        self.lang_title.pack(pady=(0, 5), anchor="w", padx=25)

        self.lang_var = ctk.StringVar(value="Hindi")
        ...
        self.lang_menu.pack(pady=(0, 15), padx=25, fill="x")

        self.btn_translate = ctk.CTkButton(
            self.right_frame, text="Translate", height=45,
            font=ctk.CTkFont(size=14, weight="bold"),
            command=self.translate_sentence
        )
        self.btn_translate.pack(pady=(0, 15), padx=25, fill="x")

        self.translated_title = ctk.CTkLabel(self.right_frame, text="Translated Text", font=ctk.CTkFont(size=14), text_color="gray")
        self.translated_title.pack(pady=(0, 5), anchor="w", padx=25)

        # Use a font family with broad Indic script coverage where available.
        self.translated_text = ctk.CTkTextbox(
            self.right_frame, height=120,
            font=ctk.CTkFont(family="Nirmala UI", size=20),
            corner_radius=12, border_width=1, border_color=("#cccccc", "#444444"),
            fg_color=("#f0f9ff", "#101820")
        )
        self.translated_text.pack(fill="x", padx=25, pady=(0, 25))
    ...
    def translate_sentence(self):
        text = self.sentence_text.get("1.0", "end-1c").strip()
        self.translated_text.delete("1.0", "end")
        if not text:
            return

        lang_code = INDIAN_LANGUAGES.get(self.lang_var.get(), "hi")
        try:
            result = GoogleTranslator(source="en", target=lang_code).translate(text)
            self.translated_text.insert("end", result if result else "(no translation returned)")
        except Exception as e:
            self.translated_text.insert("end", f"(translation failed: {e})")
    ...
    def update_frame(self):
        ...
        if not ret:
            ...
            if self.camera_failure_count > 30:
                self.camera_status_var.set("Camera disconnected or unavailable.")
            self.window.after(10, self.update_frame)
            return

        # Camera is working again after a hiccup
        self.camera_failure_count = 0
        ...
        if self.auto_add_var.get():
            ...
            if self.hold_count == 30:
                ...
                # Cooldown so it doesn't spam: requires another 50 held
                # frames (30 + 20) before it can fire again.
                self.hold_count = -20
    ...
    def on_closing(self):
        """Clean shutdown: release camera and mediapipe resources before closing the window."""
        try:
